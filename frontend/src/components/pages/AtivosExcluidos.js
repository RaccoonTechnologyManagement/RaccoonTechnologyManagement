...
import { excluidos } from '../data/ExcluidosDatabase'
...
function AtivosInicial (){
    ...
    const cabecalho = [
        'N° Pat','Categoria','Marca','Modelo','Empresa','Sede','Data Exclusão',
      ]
      const itemsPerPage = 15;
      const totalPages = Math.ceil(excluidos.length / itemsPerPage);
      ...
      const verificarSearch = excluidos 
      .filter((item) =>{
            ...
            <tbody >
              ...
              .slice(startIndex, endIndex).map((item,index) =>(
                <tr key={index}>
                  ...
                  <td className={styles.tabelaCabecalhoItens}>{item.status}</td>
